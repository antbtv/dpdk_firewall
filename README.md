...
Платформа: Raspberry Pi 5 (ARM Cortex-A76, 4 ядра, 8 ГБ RAM).
NIC WAN: Intel I210 (PCI 0001:01:00.0, igb). NIC LAN: BCM54213PE (eth0, macb).
DPDK: 24.11.3. Ядро: 6.17.0-1008-raspi (aarch64).

Топология стенда:
```
Dev PC (10.99.0.1) -- enP1p1s0 -- RPi5 -- eth0 -- Raspberry Pi 4 (10.99.0.2)
```

Команда iperf3: `iperf3 -c 10.99.0.2 -t 30` (с Dev PC).

### Накладные расходы ACL и DDoS-детектора (AF_XDP, 2026-03-19)

| Сценарий | Правил | DDoS | Мбит/с | Ретрансмиты |
|----------|--------|------|--------|-------------|
| Базовый (нет правил) | 0 | выкл | 882 | 0 |
| 10 правил DROP | 10 | выкл | 873 | 0 |
| 100 правил DROP | 100 | выкл | 846 | 0 |
| Базовый + DDoS | 1 | вкл | 842 | 0 |

Накладные расходы движка правил (0–100 правил) и DDoS-детектора — в пределах
погрешности измерения (~5% дневное колебание на RPi5). Узкое место — физический
канал 1 Гбит/с, а не CPU.

### Задержка (AF_XDP, 2026-03-14)

Инструмент: `ping -i 0.002 -c 2000`. Фоновая нагрузка: UDP iperf3.

| Нагрузка | p50 RTT | p99 RTT | Jitter (p99-p50) |
|----------|---------|---------|------------------|
| 0% | 0.175 мс | 0.207 мс | 0.032 мс |
| 10% (89 Мбит/с) | 0.173 мс | 0.355 мс | 0.182 мс |
| 90% (805 Мбит/с) | 0.420 мс | 0.783 мс | 0.363 мс |

При 90% нагрузки p99 остаётся ниже 1 мс — forwarding-путь стабилен в условиях,
близких к насыщению канала.

### DDoS-детекция (2026-03-14)

Конфигурация: `window_ms=100`, `syn_threshold=100`, `udp_threshold=500`, `block_duration_s=30`.
Инструмент: `hping3 --flood`.

| Тип атаки | Время обнаружения | Процент отброшенных |
|-----------|-------------------|---------------------|
| SYN flood (hping3 -S --flood -p 80) | не более 100 мс | 99.9% |
| UDP flood (hping3 --udp --flood -p 9999) | не более 100 мс | 99.9% |

Время обнаружения равно длине скользящего окна (`window_ms`): при скорости hping3 >1 Mpps
порог (100 SYN или 500 UDP пакетов) превышается в течение первого же окна.
После блокировки проверка чёрного списка (`rte_hash`, O(1)) отбрасывает
пакеты до попадания в движок правил. ~200 пакетов TX — не-IP кадры (ARP и др.),
пересылаемые без фильтрации.

### Эволюция производительности

| PMD | Конфигурация | Мбит/с | Ретрансм./10с | Задержка (ping avg) |
|-----|-------------|--------|---------------|---------------------|
| AF_PACKET | framecnt=512 | ~12 | 2333+ | 0.85 мс |
| AF_PACKET | framecnt=4096, qdisc_bypass=1 | ~10.5 | ~7300 | ~0.85 мс |
| AF_XDP | force_copy=1 | **895** | **0** | **0.35 мс** |
| AF_XDP | zero-copy (без force_copy) | 0 (не работает) | — | — |

Итоговая конфигурация: `net_af_xdp0,iface=enP1p1s0,force_copy=1` (WAN) + `eth_af_packet0,iface=eth0` (LAN).
Улучшение относительно AF_PACKET: **×89** (89.5% от физической скорости канала 1 Гбит/с).

### Утилизация CPU при 882 Мбит/с (2026-03-19)

| lcore | Роль | %usr | %sys | %idle |
|-------|------|------|------|-------|
| 0 | Управляющая плоскость | 0% | 0% | ~100% |
| 1 | LAN->WAN (AF_PACKET) | ~10% | ~90% | 0% |
| 2 | WAN->LAN (AF_XDP) | ~99% | ~1% | 0% |
| 3 | Резерв | 0% | 0% | ~100% |

lcore 2: 99% пользовательского времени — AF_XDP работает в busy-poll без syscall на
каждый пакет. lcore 1: 90% системного времени — AF_PACKET требует вызова ядра на каждый
burst. Оба пересылающих lcore загружены на 100% — узкое место физический канал, не CPU.
Cortex-A76 на 2.4 ГГц достаточен для насыщения гигабитного канала в режиме AF_XDP.
