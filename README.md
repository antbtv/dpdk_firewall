...
NAPI, копирование в ring AF_PACKET.
...
**H1 — тюнинг AF_PACKET** (`framecnt=4096`, `qdisc_bypass=1`): результат отрицательный. Узкое место — путь
...
в `ns_recv`. Raspi4 исключён из пути. Ранние версии топологии: dev pc (client) -> dpdk(rapberry pi 5) -> raspberry pi 4 (server). но на стороне rpi4 был bottleneck.
...
deterministic distribution, 60 секунд. Конфиги: `config/traf/traf_hairpin_*.yml`. Инструмент был создан на кафедре ВМСС.
